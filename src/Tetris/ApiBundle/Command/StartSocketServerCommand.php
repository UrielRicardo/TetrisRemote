...
namespace Tetris\ApiBundle\Command;
...
use Tetris\ApiBundle\SocketServer;	
...
class StartSocketServerCommand extends ContainerAwareCommand
{
	...
	{
		$this
			->setName('tetris:server:start')
			->setDescription('Start the Tetris socket server')
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$server = IoServer::factory(
		    new HttpServer(
		        new WsServer(
		            $this->getContainer()->get('tetris.socket_server')
		        )
		    ),
		    $port
		);
